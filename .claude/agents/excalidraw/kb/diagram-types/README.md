...
| Data Modeling | star-schema | `star-schema.md` | fan-out, convergence, evidence-card, group-container | `./star_schema_v2.png` |
| Data Modeling | snowflake-schema | `snowflake-schema.md` | star's set + tree-hierarchy + linear-pipeline | `./snowflake_schema.png` |
| Data Modeling | er | `er.md` | compartmented-box, relationship-endpoint, notation-conventions | `./er_retail_orders.png` |
| Data Modeling | data-vault | `data-vault.md` | group-container, fan-out, tree-hierarchy, convergence | `./data_vault_sales.png` |
| UML | sequence | `sequence.md` | lifeline-activation, notation-conventions | `./sequence_login_flow.png` |
| UML | class | `class.md` | compartmented-box, relationship-endpoint, notation-conventions | `./class_order_domain.png` |
| UML | use-case | `use-case.md` | group-container, notation-conventions | `./use_case_checkout.png` |
| UML | activity | `activity.md` | linear-pipeline, decision-branch, decision-marker, feedback-loop, group-container, task-list | `./activity_order_fulfillment.png` |
...
**Subject:** the pre-v1.1 legacy data-model example — `./star_schema.excalidraw` and its rendered PNG `./example_star_schema.png` (both now in this folder).
...
**Phase 6 resolution (DM-01):** the compliant canonical star example is now authored — `./star_schema_v2.excalidraw` + `./star_schema_v2.png` — and the resolver row points at it. The legacy `star_schema.excalidraw` / `example_star_schema.png` remain on disk (in this folder) as historical artifacts but are **de-indexed** as the star canonical (per RESEARCH Open Question 1 resolution).

## Adding a diagram type

Mirrors the `../patterns/README.md` "Adding a new pattern" procedure:

1. Create `diagram-types/<type>.md` with sections: *Purpose*, *How to draw it*, *Composes (primitive layer)* (`@../patterns/<pattern>.md` links — never re-derive geometry), and *Ground truth* (`@./<name>.png`).
2. Add the canonical example **pair right here in this folder** — `./<name>.excalidraw` source + rendered `./<name>.png` (same basename, side by side) — and confirm it passes the full validate → render → verify loop before indexing.
3. Add **exactly ONE** row to the resolver table above, under the correct family, naming the type file, the composed `patterns/` sub-patterns, and the example PNG.
