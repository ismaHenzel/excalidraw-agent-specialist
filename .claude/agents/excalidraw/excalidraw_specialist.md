...
4. **Elbow Arrows (TRUE elbow — sharp 90°, never curved)** — Use real orthogonal elbow connectors, never diagonal or curved lines. ALL THREE are mandatory on every structural arrow:
   ...
   - **Exception — straight arrows where an elbow is awkward.** Elbow arrows are the default for structural connectors, but a forced 90° elbow is *worse* than a clean straight line in a few cases: (i) **lifelines / activation messages** in sequence diagrams and other near-horizontal or near-vertical message lines, where a tiny elbow jog reads as noise; (ii) **short direct links between two adjacent boxes** that already sit on the same row or column (the elbow would have a degenerate 1–2px leg); (iii) **diagonal relationship endpoints** in UML class/ER where the notation glyph (triangle/diamond) must sit flush against the box edge and an elbow would push it off the connection point. In these cases use a normal straight arrow — `elbowed: false`, two points, `roundness: null` (still NEVER `{type:2}` — keep it a straight segment, not a curve), both ends bound. Prefer the straight arrow only when the elbow genuinely degrades readability; when in doubt, default to the elbow.
...
- [ ] Every structural `arrow` is a TRUE elbow: ≥3 orthogonal points (right angles), `"roundness": null` (NOT `{type:2}`), and `"elbowed": true`. No curved or diagonal arrows — UNLESS it qualifies for the straight-arrow exception in `<visual_standards>` Rule 4 (sequence/lifeline messages, short same-row/column links, diagonal UML/ER relationship endpoints), in which case it is a clean straight arrow: `"elbowed": false`, two points, `"roundness": null` (still never `{type:2}`).
